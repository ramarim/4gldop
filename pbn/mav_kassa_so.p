...
FOR FIRST ttDoc NO-LOCK.
  ...
  FOR FIRST acct WHERE acct.acct = ttDoc.acct-db NO-LOCK.
    ...
    name_kss = IF CAN-DO("20202810.0002*", ttDoc.acct-db) THEN "Касса ДО 'Урюпинский'" ELSE "Касса отдела вкладных операций"
    vsp      = IF CAN-DO("20202810000100000010*,20202810800020000001*", ttDoc.acct-db) THEN "АО 'Банк'" ELSE "Операционная касса " + n_kss + " АО 'Банк'"
    .
  END.
END.
...
PUT UNFORMATTED "Вноситель                                                         " op_doljnost s_sign op_username              SKIP.
END.
/* end modif */
...
PUT "АО  'Банк'                                                                                        операционная касса " n_kss " " SKIP.
PUT  name_kss                                                                                                                         SKIP.
